...
def seed_database():
    ...
    with app.app_context():
        ...
        db.session.commit()
        print("Admin account created! (admin@gym.com / admin123)")
        
        # 3. Seed Mock Gym Members (10 profiles)
        print("Seeding mock members with diverse membership states...")
        ...
        for index, m in enumerate(members_data, start=1):
            # Calculate join date based on months_ago
            # Safe calendar back calculation
        ...
        db.session.commit()
        print("Database successfully seeded with 10 members and mock stats!")
